...
// MAGIC %md #TPC-DS data generator to run as a job
// MAGIC
// MAGIC ## How to use it
// MAGIC 1. Attach to a cluster that you have "can manage" permission.
// MAGIC 2. Run "Cmd 2". This will create a drop down box for you to choose various Scale Factor for the TPC-DS data generation.
// MAGIC 3. Pick the desired scale factor.
// MAGIC 4. Larger scale factor will take longer to complete. We recommend attaching to the following cluster spec for each scale factor.
// MAGIC 5. Always use fixed sized cluster; do not use Autoscaling for the cluster.
// MAGIC <pre>
// MAGIC    Scale Factor      Cluster Spec (worker spec i3.2xlarge for AWS; Standard_E8ds_v4 for Azure)
// MAGIC    1                 Worker:  4 workers
// MAGIC    10                Worker:  4 workers
// MAGIC    100               Worker:  8 workers
// MAGIC    1000              Worker: 16 workers
// MAGIC    10000             Worker: 32 workers
// MAGIC </pre>
// MAGIC  5. Click the "Run All"
// MAGIC  6. Once the notebook finishes, you should see the database tpcds_sf<scale factor>_delta in the Data Explorer
// MAGIC   
// MAGIC ### Troubleshooting
// MAGIC 1. If something went wrong and you need to do a clean data generation, switch "Overwrite Data" to true and then click "Run All".

// COMMAND ----------

// MAGIC %python
// MAGIC dbutils.widgets.dropdown("benchmarks", "TPCH", ["TPCH", "TPCDS"], "Benchmarks")
// MAGIC dbutils.widgets.dropdown("scaleFactors", "1", ["1", "10", "100", "1000", "10000"], "Scale Factor")
// MAGIC dbutils.widgets.dropdown("overwrite", "false", ["true", "false"], "Overwrite Data")
// MAGIC dbutils.widgets.text("calalog", "tpch")
// MAGIC dbutils.widgets.text("baseLocation", "dbfs:/Serverless_Benchmarking")
...
val baseLocation = tryGetWidget("baseLocation").getOrElse("dbfs:/Serverless_Benchmarking") // S3 bucket, blob, or local root path
...
val mySet = benchmarks
val myString = mySet.mkString(", ")
println(myString) // "TPCH"
...
def createDatabase(databaseName: String, location: String) = {
  if (onlyTextFiles) {
    ...
    val catalog = benchmarks.mkString(", ").toLowerCase() 
    sql(s"CREATE CATALOG IF NOT EXISTS $catalog")
    sql(s"USE CATALOG $catalog")          
    if(overwrite) time(s"drop-cascade_$databaseName", sql(s"DROP DATABASE IF EXISTS $catalog.$databaseName CASCADE"))
    log(s"Creating schema $catalog.$databaseName at $location") 
...
def writeWithRetries[R](fn: => R, retries: Int = 1, benchmark: String, tableName: String, path: String) = {
  var tries = retries
  while(tries >= 0) {
    try{
      fn
