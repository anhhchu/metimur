...
# MAGIC
# MAGIC ### Depedencies
# MAGIC <li> Ensure that you either get a token as a secret or use a cluster with the env variable called pat to authenticate to DBSQL
...
pat = dbutils.notebook.entry_point.getDbutils().notebook().getContext().tags().get("browserHostName").get()
...
query_profiler.build_query_history_profile(pat = pat, mode='auto', lookback_period_days=lookbackPeriod)
